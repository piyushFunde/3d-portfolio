...
function formatNumber(num: number, formatted: boolean): FormatNumberResult {
  ...
  if (formatted) {
    if (safeNum < 1000) {
      return { number: [safeNum.toString()], unit: '' };
    }
    const units = ['k', 'M', 'B', 'T'];
    let unitIndex = 0;
    let n = safeNum;
    ...
    return { number: safeNum.toLocaleString('en-US').split(','), unit: '' };
  }
}
...
function GitHubStarsButton({
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/${username}/${repo}`)
      .then((res) => (res.ok ? res.json() : { stargazers_count: 0 }))
      .then((data) => setStars(typeof data?.stargazers_count === "number" ? data.stargazers_count : 0))
      .catch(() => setStars(0))
